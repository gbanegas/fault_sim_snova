...
void write_signatures_to_csv(const char *filename, gf16m_t X_in_GF16Matrix_fault[n_SNOVA][rank]) {
    FILE *fp;
    fp = fopen(filename, "w");
...
void sign_digest_core_ref(uint8_t *pt_signature, const uint8_t *digest, uint64_t bytes_digest, uint8_t *array_salt,
    ...
    do {
        ...
        for(int j = 0; j < sq_rank;j++) {
            printf("{ ");
            for(int i = 0; i < n_SNOVA; i++) {
                printf("%d, ", X_in_GF16Matrix_faulty[j][i]);
            }
            printf("},\n");
        }*/
    ...
    /*printf("\n\nX:\n");
    for(int i = 0; i < lsq_SNOVA; ++i) {
        printf("{");
        for(int j = 0; j < n_SNOVA;++j) {
            printf("%d, ", X_in_GF16Matrix[i][j]);
        }
        printf("}\n");
    }
    printf("\n");*/
