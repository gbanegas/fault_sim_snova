 ...
 */
void shake256(const uint8_t *pt_seed_array, int input_bytes,
		uint8_t *pt_output_array, int output_bytes) {
...
// }
void convert_GF16s_to_bytes(uint8_t *byte_array, const gf16_t *gf16_array,
		int num_of_GF16s) {
	int i;
	int pairs = num_of_GF16s >> 1;
 ...
 * @param num_of_GF16s - GF16 amount
 */
void convert_bytes_to_GF16s_cut_in_half(const uint8_t *byte_array,
		gf16_t *gf16_array, int num_of_GF16s) {
...
void gen_a_FqS(gf16_t *c, gf16m_t pt_matrix) {
	...
	gf16m_scale(S[rank - 1],
			(c[rank - 1] != 0) ? c[rank - 1] : 16 - (c[0] + (c[0] == 0)), temp);
	gf16m_add(pt_matrix, temp, pt_matrix);
	SNOVA_CLEAR(temp);
}
...
void gen_a_FqS_ct(gf16_t *c, gf16m_t pt_matrix) {
	...
	uint8_t val = zero * c[rank - 1]
			+ (1 - zero) * (15 + ct_gf16_is_not_zero(c[0]) - c[0]);
 ...
 * createHashOut
 */
void createSignedHash(const uint8_t *digest, uint64_t bytes_digest,
		const uint8_t *pt_public_key_seed, const uint8_t *array_salt,
		uint8_t *signed_hash_out) {
	Keccak_HashInstance hashInstance;
	Keccak_HashInitialize_SHAKE256(&hashInstance);
	Keccak_HashUpdate(&hashInstance, pt_public_key_seed,
			8 * seed_length_public);
 ...
 */
int verify_signture_ref(const uint8_t *pt_digest, uint64_t bytes_digest,
	...
	Keccak_HashInstance hashInstance;
	Keccak_HashInitialize_SHAKE256(&hashInstance);
	Keccak_HashUpdate(&hashInstance, pk_stru->pt_public_key_seed,
			8 * seed_length_public);
...
void evaluation_part(gf16m_t *Aalpha, gf16m_t *Balpha, gf16m_t *Qalpha1,
		gf16m_t *Qalpha2, gf16m_t (*F11)[24][24], gf16m_t Left[16][24],
		gf16m_t Right[16][24], gf16m_t X_in_GF16Matrix[29],
		...
		for (int alpha = 0; alpha < lsq_SNOVA; ++alpha) {
			for (int j = 0; j < v_SNOVA; ++j) {
				for (int k = 0; k < v_SNOVA; ++k) {
					...
					gf16m_add(Fvv_in_GF16Matrix[i], gf16m_temp1,
							Fvv_in_GF16Matrix[i]);
					/*
...
void write_signatures_to_csv(const char *filename,
		gf16m_t X_in_GF16Matrix_fault[n_SNOVA][rank]) {
	FILE *fp;
	fp = fopen(filename, "w");
...
void add_last_column(gf16_t Gauss[80][81], gf16m_t Fvv_in_GF16Matrix[5]) {
	for (int i = 0; i < m_SNOVA; ++i) {
		for (int j = 0; j < rank; ++j) {
			for (int k = 0; k < rank; ++k) {
				int index1 = i * lsq_SNOVA + j * rank + k;
				int index2 = m_SNOVA * lsq_SNOVA;
				Gauss[index1][index2] = gf16_get_add(Gauss[index1][index2],
...
void clean_temp(gf16_t Temp[16][16]) {
 ...
 */
bool sign_digest_core_ref_attack(uint8_t *pt_signature, const uint8_t *digest,
		uint64_t bytes_digest, uint8_t *array_salt, Aalpha_t Aalpha,
		Balpha_t Balpha, Qalpha1_t Qalpha1, Qalpha2_t Qalpha2, T12_t T12,
		F11_t F11, F12_t F12, F21_t F21, uint8_t *vinegar_in_byte,
...
bool sign_with_fault_injection(uint8_t *pt_signature, const uint8_t *digest,
	...
	Keccak_HashInstance hashInstance;
	Keccak_HashInitialize_SHAKE256(&hashInstance);
	Keccak_HashUpdate(&hashInstance, pt_private_key_seed,
